...
class PlayState extends MusicBeatState
{
	...
	var mispellesEngineWatermark:FlxText;
	...
	override public function create()
	{
		...
		mispellesEngineWatermark = new FlxText(4,healthBarBG.y + 50,0,SONG.song + " " + (storyDifficulty == 2 ? "Hard" : storyDifficulty == 1 ? "Normal" : "Easy") + (Main.watermarks ? " - ME " + MainMenuState.mispellesEngineVer : ""), 16);
		mispellesEngineWatermark.setFormat(Paths.font("vcr.ttf"), 16, FlxColor.WHITE, RIGHT, FlxTextBorderStyle.OUTLINE,FlxColor.BLACK);
		mispellesEngineWatermark.scrollFactor.set();
		add(mispellesEngineWatermark);

		if (FlxG.save.data.downscroll)
			mispellesEngineWatermark.y = FlxG.height * 0.9 + 45;
		...
		mispellesEngineWatermark.cameras = [camHUD];
		if (loadRep)
	...
	override public function update(elapsed:Float)
	{
		...
		if (executeModchart && luaModchart != null && songStarted)
		{
			...
			if (luaModchart.getVar("showOnlyStrums",'bool'))
			{
				...
				mispellesEngineWatermark.visible = false;
			...
			else
			{
				...
				mispellesEngineWatermark.visible = true;
		...
		private function keyShit():Void // I've invested in emma stocks
			{
				...
				if (pressArray.contains(true) && /*!boyfriend.stunned && */ generatedMusic)
				{
					...
					hitSound = FlxG.sound.load(Paths.sound('Note_Hit_Sound'), 0.5);
					...
					else if(!FlxG.save.data.hitsound)
							{
								hitSound.play(false);
							}
